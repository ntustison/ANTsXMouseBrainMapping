...
# template_ids = tuple(reversed(("P04", "P14", "P56")))
...
# time_points = np.flip(-1.0 * np.log(np.array((11.5, 13.5, 15.5, 18.5, 23, 33, 75))))


# Extract points from canny image of first template

fixed_labels_file = annotation_directory + "P56x" + template_ids[0] + "_LABELS.nii.gz"
fixed_labels = ants.image_read(fixed_labels_file)
# fixed_labels = ants.resample_image(fixed_labels, (0.1, 0.1, 0.1), use_voxels=False, interp_type=0)

# number_of_points_per_label = (1000, 1000, 1000, 10000, 10000)
...
for i in range(1, len(template_ids)):
    ...
    output_registration_prefix = output_directory + "P56x" + source_template_id + "x" + target_template_id + "_"
