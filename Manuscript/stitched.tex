...
DevCCF, and a template-based deep learning pipeline for brain extraction
and parcellation requiring minimal annotated data. In addition, we
include two modular pipelines for aligning multiplexed error-robust
fluorescence in situ hybridization (MERFISH) and fMOST datasets to the
Allen CCFv3. These workflows were adapted and tailored using ANTsX tools
to support collaborative efforts within the BICCN and are now made
openly available in a reproducible format. To facilitate broader
adoption, we also provide general guidance for customizing these
...
processing steps in various workflows\textsuperscript{30,45--48},
particularly its pairwise, intensity-based image registration
capabilities\textsuperscript{49} and bias field
correction\textsuperscript{50}. Historically, ANTsX development is based
on foundational approaches to image mapping\textsuperscript{51--53},
especially in the human brain, with key contributions such as the
Symmetric Normalization (SyN) algorithm\textsuperscript{49}. It has been
independently evaluated in diverse imaging domains including multi-site
brain MRI\textsuperscript{54}, pulmonary CT\textsuperscript{55}, and
multi-modal brain tumor registration\textsuperscript{56}.

Beyond registration, ANTsX provides functionality for template
generation\textsuperscript{57}, intensity-based
segmentation\textsuperscript{58}, preprocessing\textsuperscript{50,59},
and deep learning\textsuperscript{44}. It has demonstrated strong
performance in consensus labeling\textsuperscript{60}, brain tumor
segmentation\textsuperscript{61}, and cardiac motion
estimation\textsuperscript{62}. Built on the Insight Toolkit
(ITK)\textsuperscript{63}, ANTsX benefits from open-source contributions
...
diffeomorphic velocity model\textsuperscript{64}, inspired by previous
work\textsuperscript{65}. Second, we present a deep learning pipeline
...
datasets\textsuperscript{66,67}, and 2) a template-derived parcellation
...
longitudinal external dataset\textsuperscript{68}, demonstrating
...
brain\textsuperscript{48}. The pipeline maps spatial gene expression
...
protocols\textsuperscript{48}. Brains were placed into an optimal
...
Cellpose\textsuperscript{69,70} based on DAPI and PolyT stains which was
...
AllenCCFv3 regions. As previously reported\textsuperscript{48}, of the
...
mapping\textsuperscript{12,15,16,57,71--74}. The atlas was constructed
...
correction\textsuperscript{50}. A one-time, annotation-driven alignment
...
sparse transgenic labeling of neuron populations\textsuperscript{75,76}
using the high-throughput fMOST platform\textsuperscript{77,78}. Voxel
...
community\textsuperscript{79}, comprising symmetric, multi-modal MRI and
LSFM templates generated using the ANTsX framework\textsuperscript{57}.
...
system\textsuperscript{80}. By dividing the brain into anatomically or
...
variation in gene expression patterns\textsuperscript{81,82}. While deep
learning techniques have yielded robust segmentation and labeling tools
for the human brain (e.g., SynthSeg\textsuperscript{83},
ANTsXNet\textsuperscript{44}), analogous development for mouse data
(e.g., MEMOS\textsuperscript{84}) has been limited. Mouse neuroimaging
...
segmentation (i.e., brain extraction) and the subsequent atlas-based
region segmentation.
...
the University of North Carolina at Chapel Hill\textsuperscript{66},
includes 16 isotropic MRI volumes acquired at
\(0.16 \times 0.16 \times 0.16\) mm\(^3\) resolution. The second
dataset\textsuperscript{67} comprises 88 specimens acquired in three
...
volumes using a B-spline fitting algorithm\textsuperscript{85}. Using
this synthesized dataset and the CAMRI images, we created two
ANTsX-based population templates\textsuperscript{57}, each paired with a
...
dataset\textsuperscript{68} that is completely independent from the data
...
bias field correction algorithm\textsuperscript{50}, originally
...
denoising\textsuperscript{59} has been implemented as
...
algorithm\textsuperscript{49}, a symmetric diffeomorphic model with
optional B-spline regularization\textsuperscript{65}. In ANTsPy,
...
subjects\textsuperscript{57}. This functionality was used in generating
...
the entire brain\textsuperscript{76}. Because these reconstructions are
...
registration\textsuperscript{64}, where a velocity field governs the
...
datasets\textsuperscript{66,67}. One of the templates was constructed
...
parameters\textsuperscript{68}. Although each label or imaging modality
