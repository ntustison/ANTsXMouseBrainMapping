...
Space.\textsuperscript{14} Despite the significances of these
...
complicate assumptions of mapping correspondence for conventional
...
imaging methodologies. For example, if tissue deformations are not a
problematic artifact, DeepSlice can be used to determine affine
mappings\textsuperscript{35} with the computational efficiency
associated with neural networks.

As noted above, many of the existing approaches for image data
processing of mouse brain use ANTsX tools for core steps in various
workflows, particularly its pairwise, intensity-based image registration
tools and bias field correction. Historically, ANTsX development is
originally based on fundamental approaches to image
mapping\textsuperscript{36--38} which resulted in such core
contributions to the field as the Symmetric Normalization (SyN)
...
(e.g., multi-site brain MRI data,\textsuperscript{40} pulmonary CT
data),\textsuperscript{41} and most recently multi-modal brain
registration in the presence of tumors.\textsuperscript{42}

\input{antsx_functionality_table}

Apart from its founding contribution, ANTsX is a comprehensive
...
Insight Toolkit (ITK) deriving benefit from a very capable open-source
community of scientists and programmers as well as providing a venue for
...
resource.\textsuperscript{46} These symmetric atlases, comprising both
...
E15.5, E18.5 and postnatal day (P) 4, P14, and P56 with Allen CCFv3
integration with the P56 template. Modalities include at least four MRI
contrasts and light sheet flourescence miscroscopy (LSFM) per
developmental stage. Gene expression and other cell type data were
mapped to the corresponding developmental time point to guide the
associated anatomical parcellations. In addition to conventional
preprocessing (e.g., bias field correction, brain extraction)
difficulties for the various mapping tasks included tissue distortion,
missing slices, and acquisition artifacts. We illustrate the utility of
the ANTsX ecosystem in providing solutions for these issues in
constructing the DevCCF. In addition, we also demonstrate ANTsX
functionality in the generation of a single diffeomorphic transformation
model\textsuperscript{47} between developmental stages where mappings
between any two continuous time points within the temporal span of the
E11.5 and P56 atlases is determined by integration of the governing
time-varying velocity field.\textsuperscript{48} Such transformations
permit the possibility of ``pseudo'' templates generated between
available developmental stages, which we also demonstrate.
...
well-performing patch-based denoising technique\textsuperscript{49} and
...
nonlinearities.\textsuperscript{25,50} These augmentation techniques are
...
for the developmental brain atlas work\textsuperscript{46} we developed
...
architecture\textsuperscript{51} and perform prediction in a slice-wise
...
brain extraction networks, a 2-D U-net architecture\textsuperscript{51}
...
network\textsuperscript{52} was ported to ANTsXNet and expanded to 3-D
for various super-resolution applications,\textsuperscript{53} including
...
described\textsuperscript{55} and recently used to create the DevCCF
templates.\textsuperscript{46} An initial template estimate is
...
Miller.\textsuperscript{47} Integration of the resulting velocity field
...
templates,\textsuperscript{46} we coalesced the manual parcellations of
