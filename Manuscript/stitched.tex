...
We also describe a structural morphological pipeline for calculating
image-based cortical thickness measurements\textsuperscript{52}
analogous to the well-known and heavily utilized ANTsX cortical
thickness pipeline for cross-sectional and longitudinal human brain
studies.\textsuperscript{26,53,54} Two integral pipeline components are
brain extraction and brian parcellation derived from, respectively,
two-shot and single-shot deep learning, leveraging only publicly
available resources, including AllenCCFv3 and DevCCF. Although we
...
schemes. Evaluation is performed on an independent open-source data
set\textsuperscript{55} comprising longitudinal acquisitions of multiple
specimens.
...
well-performing patch-based denoising technique\textsuperscript{56} and
...
nonlinearities.\textsuperscript{26,57} These augmentation techniques are
...
architecture\textsuperscript{58} and perform prediction in a slice-wise
...
architecture\textsuperscript{58} was created to separate the background
and foreground.
...
network\textsuperscript{59} was ported to ANTsXNet and expanded to 3-D
for various super-resolution applications,\textsuperscript{60} including
...
capable of excellent performance.\textsuperscript{41,62} Various
