...
WholeBrain.\textsuperscript{34} Relatedly,
FriendlyClearMap\textsuperscript{35} and
mBrainAligner\textsuperscript{36} are both landmark-based approaches to
mapping of the mouse brain. Whereas the former employs
Elastix\textsuperscript{37} functionality, the latter is based on
developed methodology referred to as \emph{coherent landmark mapping}.
Finally, the widespread adoption of deep learning techniques has also
influenced development in mouse brain imaging methodologies. For
example, if tissue deformations are not considered problematic for a
particular dataset, DeepSlice can be used to determine affine
mappings\textsuperscript{38} with the optimal computational efficiency
...
mapping,\textsuperscript{39--41} particularly in the human brain, which
has resulted in core contributions to the field such as the well-known
Symmetric Normalization (SyN) algorithm.\textsuperscript{42} Since its
...
data,\textsuperscript{43} pulmonary CT data,\textsuperscript{44} and
most recently, multi-modal brain registration in the presence of
tumors.\textsuperscript{45}

Apart from its registration capabilities, ANTsX comprises additional
functionality such as template generation,\textsuperscript{46}
intensity-based segmentation,\textsuperscript{47}
preprocessing,\textsuperscript{25,48} deep learning
...
consensus labeling,\textsuperscript{49} brain tumor
segmentation,\textsuperscript{50} and cardiac motion
estimation\textsuperscript{51} ). Importantly, ANTs is built on the
Insight Toolkit (ITK)\textsuperscript{52} deriving benefit from the
...
transformation model,\textsuperscript{53} influenced by previous
work.\textsuperscript{54} The resulting time-parameterized velocity
...
algorithm,\textsuperscript{55} this was later expanded to include a
complete processing framework for human brain cortical thickness
estimation for both cross-sectional\textsuperscript{56} and
longitudinal\textsuperscript{57} data using T1-weighted MRI. These
...
templates generated from two open datasets\textsuperscript{58,59} and
...
performed on an independent open dataset\textsuperscript{60} comprising
...
micro-optical sectioning tomography (fMOST)\textsuperscript{61,62}
platform was used to image 55 mouse brains containing gene-defined
neuron populations, with sparse transgenic
expression.\textsuperscript{63,64} In short, the fMOST imaging platform
...
transcriptomic data onto AllenCCFv3\textsuperscript{65} required the
...
detailed procedure.\textsuperscript{65} Briefly, a brain of C57BL/6
...
used.\textsuperscript{65} Raw MERSCOPE data were decoded using Vizgen
software (v231). Cell segmentation was performed.\textsuperscript{66} In
...
reported,\textsuperscript{65} further assessment of the alignment showed
...
symmetric normalization).\textsuperscript{42} The latter set includes
...
symmetric normalization transform.\textsuperscript{42}
...
algorithm,\textsuperscript{55} a complete algorithmic workflow was
developed for both cross-sectional\textsuperscript{56} and
longitudinal\textsuperscript{57} T1-weighted MR image data. This
contribution was later refactored using deep
learning\textsuperscript{26} leveraging the earlier
results\textsuperscript{56} for training data.
...
Center for Animal MRI (CAMRI) dataset\textsuperscript{58} from the
...
The second high-resolution dataset\textsuperscript{59} comprises 88
...
algorithm developed in ANTsX.\textsuperscript{68} From these two
datasets, two symmetric isotropic ANTsX templates\textsuperscript{46}
were generated analogous to the publicly available ANTsX human brain
templates used in previous research.\textsuperscript{56} Bias field
...
dataset\textsuperscript{60} which is completely independent from the
...
technique\textsuperscript{48} and is also available as an image filter
...
scenarios.\textsuperscript{69} It includes innovative transformation
models for biological modeling\textsuperscript{42,54} and has proven
capable of excellent performance.\textsuperscript{43,70} Various
...
normalization (SyN) with Gaussian\textsuperscript{42} or B-spline
regularization\textsuperscript{54} where the forward transform is
...
described\textsuperscript{46} and recently used to create the DevCCF
...
algorithm.\textsuperscript{42} The template estimate is updated by
...
\item
  ...
  registration functionality\textsuperscript{69} available in ANTsX to
...
\item
  ...
  neuron projections\textsuperscript{64}. Spatial mapping of these
...
technique,\textsuperscript{68} also created by the ANTsX developers and
contributed to ITK.
...
nonlinearities.\textsuperscript{26,71} These augmentation techniques are
...
conventional U-net architecture\textsuperscript{72} and, in ANTsPyNet,
...
orthogonal multi-plane, high resolution data\textsuperscript{59} which
were combined to synthesize isotropic volumetric data using the B-spline
fitting algorithm.\textsuperscript{68} This algorithm is encapsulated in
...
\leavevmode\vadjust pre{\hypertarget{ref-Qu:2022aa}{}}%
\CSLLeftMargin{36. }
\CSLRightInline{Qu, L. \emph{et al.} Cross-modal coherent registration
of whole mouse brains. \emph{Nat Methods} \textbf{19}, 111--118 (2022).}
...
\leavevmode\vadjust pre{\hypertarget{ref-Gee:1993aa}{}}%
\CSLLeftMargin{41. }
